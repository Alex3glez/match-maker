        ...
        <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {localIsNew && localStatus === "applied" && (
              <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">Nuevo</span>
            )}
            {statusBadge}
            <div className={`flex h-12 w-12 items-center justify-center rounded-full font-bold shadow-sm ${getMatchScoreClasses(app.match_score)}`}>
              {app.match_score}%
            </div>
            <button
              type="button"
